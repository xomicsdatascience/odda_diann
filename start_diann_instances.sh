...
for i in `ls ${apptainer_path}/*.sif`; do
	...
	im_name=`basename ${i}`;
	im_name="${im_name%.*}";
	apptainer instance start -B /data/articles/:/data/articles/:ro -B /data/datasets/:/data/datasets/:ro -B /data/multistudy/:/data/multistudy/:ro -B /data/quantified/:/data/quantified/ ${i} ${im_name} 
done
